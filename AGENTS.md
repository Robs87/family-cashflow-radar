...
本仓库是用户的本地家庭现金流分析项目。目标是把貔貅记账 CSV 翻译成家庭财务决策系统。
...
```text
CSV 导入 → SQLite 原始落库 → 标准化交易 → 规则分类 → 月度现金流 → CLI 摘要 → 最简 Web 仪表盘
```
...
按 `docs/plans/mvp-implementation-plan-v0.1.md` 执行。
...
Task 0：AGENTS.md
Task 1：schema.sql
Task 2：seed_rules.sql
Task 3：synthetic CSV fixtures
Task 4：import_csv.py
Task 5：normalize.py
Task 6：classify.py
Task 7：generate_monthly_cashflow.py
Task 8：print_summary.py
