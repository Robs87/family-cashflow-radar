...
- 按 v0.2/v0.3 后续计划推进决策分析层：新增提前还贷、大额消费/买车、投资加仓的现金流压力模拟。
- Web 首页新增“决策模拟”表单和“最近模拟结果”，模拟结果写入 `decision_scenarios`，保留风险等级、最低安全垫、建议和解释。
- 当前模拟先做现金流压力判断，不预测投资收益；提前还贷的利息节省仍需结合房贷计划进一步做专门比较。
- 继续细化提前还贷模拟：在不修改真实还款计划的前提下，按当前房贷模板估算“缩短期限/降低月供”的未来利息节省和期数变化。

## 2026-05-22

- 审核并收口开发文档：`docs/prd/prd-v0.2.md` 和 `docs/plans/v0.2-action-advice-plan.md` 作为当前事实源。
...
- 落地当前现金余额校准：本地保存可用现金快照，首页展示覆盖月数，财务建议和决策模拟优先使用最新校准现金作为安全垫起点。
- 落地未来计划事件：支持计划收入 / 支出、启停、BeeCount 实际流水匹配去重，并接入决策模拟。
- 细化大额消费 / 买车模拟：支持分期、每月新增固定支出、收入影响、风险月份和建议金额上限。
- 落地投资仓位安全上限：不预测收益，只计算必须保留现金、可投资上限和是否暂缓加仓。

## 待办

- 下一阶段：基于真实使用反馈，收紧计划事件匹配规则和模拟器解释文案。
